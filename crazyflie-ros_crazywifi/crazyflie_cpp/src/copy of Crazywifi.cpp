...
Crazywifi::Crazywifi()
{
	...
	*/
	...
	const void *buf = "Welcome! TCP/IP connection initialized.";
	//while(1){
		...
		status = send(new_conn_fd,buf, 7,0);
		if(status == -1)
		{
			close(new_conn_fd);
			_exit(4);
		...
		printf("TCP/IP message received with length: %d bytes.\n",status);
	//}
    ...
    const uint8_t* data)
{
    ...
    //while(1){
        ...
        status = send(new_conn_fd,data, 7,0);
        if(status == -1)
        {
            close(new_conn_fd);
            _exit(4);
